...
class PurchaseListView(LoginRequiredMixin, TitleContextMixin, SearchQuerysetMixin, ListView):
    ...
    def get_queryset(self):
        ...
        # Asegurar que solo mostramos compras activas en la lista por defecto
...
class PurchaseCreateView(LoginRequiredMixin, TitleContextMixin, CreateView):
    ...
    fields = ['supplier', 'num_document', 'subtotal', 'iva', 'total']  # Quitamos issue_date
    ...
    def form_valid(self, form):
        form.instance.user = self.request.user
        form.instance.issue_date = timezone.now()   # Fecha exacta actual
        return super().form_valid(form)
